...
export const useFlowStore = defineStore('flow', () => {
    ...
    function saveState(description = 'Unknown action') {
        ...
        if (history.value.length > CONFIG.MAX_HISTORY_ENTRIES) {
            ...
            console.log(
                `📚 History limit reached (${CONFIG.MAX_HISTORY_ENTRIES}). Removing ${entriesToRemove} oldest entries.`,
            )
            ...
            if (hasPositionChange) {
                ...
                if (addChanges.length > 0) {
                    if (addChanges.length === 1) {
                        ...
                        const description =
                            addChanges.length === uniqueTypes.length && uniqueTypes.length === 1
    ...
    function onEdgesChange(changes) {
        ...
        // We only save history for significant, atomic actions.
        const hasSignificantChange = changes.some((change) => change.type === 'add' || change.type === 'remove')
    ...
    function removeEdgesForBranch(nodeId, branchId) {
        ...
        const edgesToRemove = edges.value.filter((edge) => {
            ...
            const matches =
                edge.data?.branchId === branchId && edge.source === nodeId && edge.data?.isConditionBranch === true
        ...
        if (edgesToRemove.length > 0) {
            ...
            edges.value = edges.value.filter((edge) => !(edge.data?.branchId === branchId && edge.source === nodeId))
