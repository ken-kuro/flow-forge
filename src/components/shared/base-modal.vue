...
<template>
    <div class="modal modal-open" @click.self="handleClose" @keydown.stop @keyup.stop @keypress.stop>
        <div
        ...
        >
            ...
            >
                ...
                <button v-if="showCloseButton" @click="handleClose" class="btn btn-sm btn-circle btn-ghost">
                    <X class="w-4 h-4" />
                </button>
            </div>

            <!-- Modal Body -->
            <div class="flex-1 flex flex-col min-h-0 overflow-auto" :class="{ 'p-6': bodyPadding }">
                <slot></slot>
            </div>

            <!-- Modal Footer - shown if showFooter is true or footer slot is used -->
            <div v-if="showFooter || $slots.footer" class="border-t border-base-300 p-6 pt-4 flex-shrink-0">
                <slot name="footer">
                    <div class="flex justify-end gap-3">
                        <button @click="handleSecondaryClick" class="btn btn-ghost">
                            {{ secondaryButtonText }}
                        </button>
                        <button @click="handlePrimaryClick" class="btn btn-primary" :disabled="primaryButtonDisabled">
                            {{ primaryButtonText }}
                        </button>
                    </div>
                </slot>
