...
<template>
    <div
        class="card-block bg-base-100 border border-base-300 rounded-lg p-3 transition-all duration-200 ease-in-out hover:border-base-content/30"
    >
        <!-- Block Header -->
        <div class="block-header flex items-center justify-between mb-3">
            <div class="flex items-center gap-2 flex-1 min-w-0">
                <component :is="icon" :class="['w-4 h-4 flex-shrink-0', iconColor]" />
                <InlineEditText v-model="title" :placeholder="placeholder" class="text-sm font-medium flex-1" />
            </div>
            <button
                @click="handleDelete"
