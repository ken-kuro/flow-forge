...
import ImageSetupToolbar from './image-setup-toolbar.vue';
    ...
    <div class="modal-box w-11/12 max-w-7xl h-[90vh] p-0 flex flex-col">
      ...
      <div class="flex-1 flex gap-0 min-h-0">
        ...
        <div class="flex-1 flex flex-col p-6">
          <div class="flex justify-between items-center mb-4">
            ...
            <ImageSetupToolbar v-model:active-tool="activeTool" />
          </div>
          <div class="flex-1 bg-base-200 rounded-lg p-4 flex items-center justify-center min-h-0 overflow-hidden">
            ...
            >
              ...
              <div v-for="(txt, index) in texts" :key="`txt-${index}`"
                   class="absolute border-2 border-primary box-border select-none pointer-events-none p-1 flex items-center justify-center"
                   :style="{
                     left: `${txt.rect.x}px`,
                     top: `${txt.rect.y}px`,
                 ...
                 <span class="absolute -top-1.5 -left-1.5 bg-primary text-primary-content text-xs px-1.5 py-0.5 rounded font-medium min-w-[1.5rem] text-center select-none">
                  {{ index + 1 }}
                </span>
                <span class="text-primary-content text-sm bg-primary/70 px-2 py-1 rounded">
                  {{ txt.text }}
                </span>
              </div>
              <!-- New drawing rect -->
      ...
      <div class="border-t border-base-300 p-6 pt-4 flex-shrink-0">
        <div class="flex justify-end gap-3">
          ...
          <button @click="saveElements" class="btn btn-primary">Save</button>
        </div>
      </div>
    </div>
  </div>
